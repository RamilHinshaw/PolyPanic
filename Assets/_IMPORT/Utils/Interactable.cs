...
public class Interactable : MonoBehaviour
{
    ...
    public enum InteractType { None, AutoRun, Activation, OnCollisionEnter, OnTriggerEnter, OnTriggerExit, OnTriggerEnter2D }
    ...
    {
        if (!enabled) return; //Prevents Reflections
        if (interactType != InteractType.OnTriggerEnter2D) return;
